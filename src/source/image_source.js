...
class ImageSource extends Evented implements Source {
    ...
    load() {
        ...
        ajax.getImage(
            this.map._transformRequest(this.url, ajax.ResourceType.Image),
            (err, image) => {
                if (err) {
            ...
            this.options.url.indexOf('access_token') === -1 &&
    ...
    setCoordinates(
        ...
        const cornerZ0Coords = coordinates.map(coord => {
            return map.transform
                .locationCoordinate(LngLat.convert(coord))
                .zoomTo(0)
        })

        // Compute the coordinates of the tile we'll use to hold this image's
        // render data
        const centerCoord = (this.centerCoord = util.getCoordinatesCenter(
            cornerZ0Coords
        ))
